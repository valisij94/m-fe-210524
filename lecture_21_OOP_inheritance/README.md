...
А теперь перейдем к конкретным животным. Рассмотрим, например, собаку, рыбку и попугая.
...
2. Теперь пусть и хоббит, и гном говорят свое имя. Хоббит должен говорить "Hi, stranger, I am hobbit _NAME_ from _HOME_VILLAGE_". Гном должен говорить "Hi, friend, I am dwarf _NAME_, son of _FATHER_NAME_". К свойству `name`, нам нужно обращаться через родителя (то есть ссылку на объект родителя, `super`).
